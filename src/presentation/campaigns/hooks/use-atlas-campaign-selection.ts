...
export function useAtlasCampaignSelection({
  ...
  const availableSidebarListMode = sidebarListMode === "updates" && contentUpdates.length === 0
    ? "locations"
    : sidebarListMode;
  const explicitlySelectedContentUpdate = contentUpdates.find(
    (contentUpdate) => contentUpdate.key === selectedContentUpdateKey,
  ) ?? null;
  const urlSelectedContentUpdate = availableSidebarListMode === "updates" && urlContentUpdateLevelId
  ...
  return {
    ...
    sidebarListMode: availableSidebarListMode,
    urlCampaignLevelId,
