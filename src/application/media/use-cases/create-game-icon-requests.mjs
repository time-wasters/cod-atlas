...
import { createMobyGamesIconRequest } from "./create-moby-games-icon-request.mjs";
...
export function createGameIconRequests(game, configuration) {
  ...
  if (game.images?.mobygames) {
    return createMobyGamesIconRequest(game, configuration.mobyGamesTemplate);
  }
  return [];
}
