---
id: cod-airfield-escape
title: Airfield Escape
games:
  - cod
mode: singleplayer
wikiArticle: codwiki-airfield-escape
locations:
  - id: main
    country: Germany
    region: Hesse
    city: Fritzlar
    landmark: Fritzlar Air Base
    latitude: 51.1145
    longitude: 9.285833
    precision: exact
    confidence: medium
    method: real-world-inspiration
    primary: true
    urls:
      - googleMaps: https://www.google.com/maps/search/?api=1&query=Bundeswehrflugplatz+Fritzlar%2C+Fritzlar%2C+Germany
      - wikipedia: https://en.wikipedia.org/wiki/Fritzlar_Air_Base
---

> **AI-generated research note:** The historical summary below was generated
> with AI assistance and should be reviewed against the cited sources before
> being treated as authoritative.

## The Mission in the Game

The mission takes place on September 2, 1944, immediately after “Eder Dam
Getaway.” Captain Price drives Sergeant Evans and Sergeant Waters onto an
unnamed airfield in Kurhesse, where the rest of their fictional 2 SAS team is
preparing a captured aircraft. Evans draws fire from the ground force and uses
an antiaircraft gun to destroy Ju 87 Stukas taxiing for takeoff before the team
drives across the tarmac and escapes by air.

## The Real Place & Differences

Fritzlar Air Base is the strongest real-world candidate. “Kurhessen” is the
historical name for northern Hesse, and Fritzlar lies in the Eder lowlands east
of the Edertalsperre. The Luftwaffe constructed the field during the military
expansion of 1935–38. Its original complex included hangars, barracks, and a
flight-operations building with a control tower, making it a substantially
better match than a generic regional or country marker.

The chronology also fits at a broad level. Junkers used Fritzlar's hangars for
aircraft maintenance and production from 1941 until October 1944, including
assembly of the Ju 352 transport. From September 1944 into March 1945, III Group
of Nachtjagdgeschwader 1 operated from Fritzlar with Bf 110 G and Ju 88 G night
fighters. The field was therefore active at approximately the mission's date
and could plausibly contain valuable aircraft and substantial ground
infrastructure.

Important details do not match. The documented September 1944 night-fighter
complement was not a formation of Ju 87 Stuka dive bombers, and no reviewed
source identifies a British raid, mass runway engagement, or aircraft theft at
Fritzlar on September 2. Although photographic evidence shows at least one Ju
87 at Fritzlar by the time American forces captured the field in 1945, that
does not establish the game's Stuka force there in 1944. The game's control
tower, runway battle, antiaircraft emplacement, and escape aircraft should be
treated as a fictional airfield composition.

## The Real Mission & Differences

The represented SAS operation is likewise unsupported. British SAS formations
did conduct sabotage, reconnaissance, resistance support, and target marking
in north-west Europe during 1944–45, but the reviewed institutional history
does not document Price, Evans, Waters, or a 2 SAS penetration from the Eder
Dam to Fritzlar. The level continues the invented commando narrative of the two
preceding missions while attaching it to a credible German military site.

## Marker Position Explanation

The marker at `51.1145, 9.285833` identifies Fritzlar Air Base itself. Its
`exact` precision describes the coordinate of the real airfield, while
`confidence: medium` and `method: real-world-inspiration` record that Fritzlar
is a researched inference rather than a canonically named location. It lies
about 17.6 kilometres from the Edertalsperre in a straight line, which is close
enough to support the campaign's geographic sequence but farther than the
preceding briefing's claimed eight miles even before road distance is counted.
The stored Google Maps URL searches for the real Bundeswehrflugplatz Fritzlar;
the atlas marker uses the curated coordinates above.

## Sources

- [Call of Duty Wiki: Airfield Escape](https://callofduty.fandom.com/wiki/Airfield_Escape)
  — mission date, represented unit, Kurhesse setting, objectives, Stukas, and
  escape sequence.
- [Call of Duty Wiki: Airfield Escape transcript](https://callofduty.fandom.com/wiki/Airfield_Escape/Transcript)
  — mission dialogue and continuity with the assembled SAS team and captured
  escape aircraft.
- [City of Fritzlar: historical timeline](https://www.fritzlar.de/leben-wohnen/stadtportrait/aus-der-geschichte/zeittafel-zur-ortsgeschichte/)
  — official municipal account of the airfield's 1935–38 construction in the
  Eder lowlands, early Luftwaffe occupation, and original control building.
- [Fritzlar history association: 75 years of Fritzlar airfield](https://gdh-fritzlar.de/downloads/02A%2075Jahre_FlPlatzFZ.pdf)
  — detailed local history of the wartime airfield, Junkers works, aircraft
  production, and September 1944 night-fighter deployment.
- [Henry L. deZeng IV: *Luftwaffe Airfields 1935–45, Germany*](https://www.ww2.dk/Airfields%20-%20Germany%20%5B1937%20Borders%5D.pdf)
  — compiled wartime airfield history, infrastructure, stationed units, and
  aircraft evidence for Fritzlar.
- [National Army Museum: Special Air Service](https://www.nam.ac.uk/explore/SAS)
  — formation of 2 SAS and the documented European role of the SAS Brigade in
  1944–45.
- [Fritzlar Air Base](https://en.wikipedia.org/wiki/Fritzlar_Air_Base)
  — consolidated history of Junkers production and III/NJG 1's Bf 110 and Ju
  88 deployment at Fritzlar.
- [OpenStreetMap: Fritzlar Air Base](https://www.openstreetmap.org/?mlat=51.1145&mlon=9.285833#map=15/51.1145/9.285833)
  — present-day airfield location used for the landmark coordinates.
