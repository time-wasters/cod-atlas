---
id: cod-eder-dam-getaway
title: Eder Dam Getaway
games:
  - cod
mode: singleplayer
campaign:
  id: "2"
  label: British Campaign
wikiArticle: codwiki-eder-dam-getaway
locations:
  - id: main
    country: Germany
    region: Hesse
    city: Affoldern
    landmark: Eder bridge at Affoldern
    latitude: 51.16506
    longitude: 9.08726
    precision: approximate
    confidence: medium
    method: real-world-inspiration
    primary: true
    urls:
      - googleMaps: https://www.google.com/maps/search/?api=1&query=Ederbr%C3%BCcke+Affoldern%2C+Br%C3%BCckenstra%C3%9Fe%2C+34549+Edertal%2C+Germany
      - wikipedia: https://en.wikipedia.org/wiki/Eder
---

> **AI-generated research note:** The historical summary below was generated
> with AI assistance and should be reviewed against the cited sources before
> being treated as authoritative.

## The Mission in the Game

The mission follows immediately after the game's attack on the Eder Dam on
September 2, 1944. Captain Price drives Sergeant Evans and Sergeant Waters away
in a captured Opel Blitz while German trucks, Kübelwagens, and motorcycles
pursue them. Evans fires Panzerfausts from the truck, then covers Waters while
he manually demolishes a wooden bridge. Their briefing orders 3 Troop, 2 SAS to
reach an airfield “approx. 8 mile away” at the map reference `033875`.

## The Real Place & Differences

Fritzlar is nevertheless a plausible inspiration for the unnamed destination.
Its Luftwaffe airfield was constructed in the Eder lowlands during the German
military expansion of 1935-38, supported Junkers aircraft production during the
war, and hosted night-fighter units in 1944-45. It is the strongest documented
wartime airfield near the dam and is already used—with medium confidence—for
the following “Airfield Escape” level. However, Fritzlar Air Base lies about
17.5 kilometres (roughly 11 miles) from the Edertalsperre even in a straight
line, with a longer journey by road. That conflicts with the briefing's eight
miles, and the bare grid reference cannot securely identify the field without a
verifiable period map and grid system.

An eastbound journey from the dam toward Fritzlar would naturally follow the
Eder valley through Hemfurth and Affoldern. Affoldern stands immediately below
the Edertalsperre and has a road crossing over the Eder on Brückenstraße. This
makes its bridge a geographically coherent real-world analogue for the level's
fixed bridge scene. The game environment—especially its timber bridge,
waterfall, tunnels, roadblocks, and pursuit sequence—does not reproduce the
present crossing closely enough to establish identity, and no evidence shows
that SAS troops destroyed the Affoldern bridge in 1944.

## The Real Mission & Differences

No reviewed historical source identifies this escape or bridge demolition as a
real operation. The premise continues the preceding level's fictional SAS raid:
the actual Edertalsperre was breached from the air by No. 617 Squadron RAF in
May 1943 and rebuilt during that year, not attacked by this commando team in
September 1944. The real SAS did conduct sabotage and reconnaissance behind
enemy lines in north-west Europe, but the available SAS history does not place
Price, Evans, Waters, or a 2 SAS bridge action in the Eder valley.

## Marker Position Explanation

The marker at `51.16506, 9.08726` therefore represents the real Eder bridge at
Affoldern on a plausible escape corridor from the dam toward Fritzlar. Its
precision remains `approximate` and its method `real-world-inspiration`: the
coordinate is accurate for the analogue, not a claim that the fictional chase
occurred there. The stored Google Maps URL searches for the named bridge and
street in Affoldern; the atlas marker uses the curated coordinates above.

## Sources

- [Call of Duty Wiki: Eder Dam Getaway](https://callofduty.fandom.com/wiki/Eder_Dam_Getaway)
  — mission date, represented unit, general setting, destination distance,
  bridge objective, vehicles, and pursuit narrative.
- [Call of Duty Wiki: Eder Dam Getaway transcript](https://callofduty.fandom.com/wiki/Eder_Dam_Getaway/Transcript)
  — the `033875` map reference, eight-mile claim, bridge-demolition order, and
  mission dialogue.
- [Royal Air Force Museum: 75th anniversary of the Dambusters](https://www.rafmuseum.org.uk/blog/75th-anniversary-of-the-dambusters/)
  — the real No. 617 Squadron air attack on the Eder during Operation Chastise.
- [German Digital Library / Federal Waterways Engineering and Research Institute: reconstruction of the Edertalsperre](https://www.deutsche-digitale-bibliothek.de/item/P2UDX25ULPQBWECT7TFE6XYW7ZYFUIOF)
  — archival evidence that the breached dam was rebuilt by the end of 1943.
- [National Army Museum: Special Air Service](https://www.nam.ac.uk/explore/SAS)
  — formation of 2 SAS and the SAS Brigade's documented European role during
  1944-45.
- [City of Fritzlar: historical timeline](https://www.fritzlar.de/leben-wohnen/stadtportrait/aus-der-geschichte/zeittafel-zur-ortsgeschichte/)
  — official municipal history placing construction of the military airfield
  in the Eder lowlands between 1935 and 1938.
- [Fritzlar Air Base](https://en.wikipedia.org/wiki/Fritzlar_Air_Base)
  — wartime Junkers production and Luftwaffe night-fighter use relevant to the
  following airfield mission.
- [OpenStreetMap: Eder bridge at Affoldern](https://www.openstreetmap.org/?mlat=51.16506&mlon=9.08726#map=18/51.16506/9.08726)
  — present-day road crossing used for the representative coordinates.
